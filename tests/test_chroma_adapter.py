...
async def test_chroma_upsert_query_delete_round_trip(tmp_path):
    ...
    await store.upsert(
        [
            ...
            record("other", [1.0, 0.0, 0.0, 0.0], scope_id="conv-2", document_id="doc-2"),
        ]
    )

    hits = await store.query(
        [1.0, 0.0, 0.0, 0.0], scope_type="conversation", scope_id="conv-1", top_k=5
    )
    ...
    assert (
        await store.query(
            [1.0, 0.0, 0.0, 0.0], scope_type="conversation", scope_id="conv-2", top_k=5
        )
        != []
    )
